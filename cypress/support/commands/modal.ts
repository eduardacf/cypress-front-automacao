...
Cypress.Commands.add('validarModal', (usuario) => {
    cy.get('.modal-content').within(() => {
        cy.contains('td', 'Student Name').next().should('contain', `${usuario.nome} ${usuario.sobrenome}`);
        ...
        cy.contains('td', 'State and City').next().should('contain', `${usuario.estado} ${usuario.cidade}`);
    });
});
